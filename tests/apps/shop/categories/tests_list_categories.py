...
class ShopCategoriesListViewTest(APITestCase):
   ...
   def test_get_list_categories(self):
      ...
      self.assertIn('Category 0', data['results'][0]['name'])
   ...
   def test_get_category_by_search_name_query_param(self):
      ...
      response = self.client.get(f'{CATEGORY_URL}?search={category.name}')
      data = response.json()
      self.assertEqual(response.status_code, status.HTTP_200_OK)
      self.assertIn(serializers.CategorySerializer(category).data, data['results'])
   ...
   def test_try_to_get_category_by_search_name_no_registered_query_param(self):
      ...
      response = self.client.get(f'{CATEGORY_URL}?search=new')
      data = response.json()
      self.assertEqual(response.status_code, status.HTTP_200_OK)
